...
I tried to make this plugin compatible with as much of the YEP library as I could, though I can't guarantee compatibility with EVERY plugin. I extended the idea of Crisis regeneration by not only allowing you to choose whether it activates at a set rate or a fixed value, but also by allowing custom stats to trigger regeneration as well as adding in Supercharged regeneration which triggers when a stat climbs high enough. This plugin has all the base features MP and TP have to make the custom stats feel like a natural extension.

Unfortunately, I can't guarantee every feature works. There's a lot of custom formulas for different situations, and I didn't test that all trigger <sub>(though since that section referenced Yanfly most heavily it should be fine??)</sub>. Custom skill costs are definitely functional. Stats are accessible as normal, simply use battler.\[custom stat\], ex. for a stat named 'ap' call `battler.ap` where battler is a Game_Battler object. A lot of features use base engine code as a template so it should work. JSON files have been tested but there is no documentation on the expected fields, so you'll just have to look at the parameter parsing code to find the fields that are expected. Script calls were also never documented, so you'll have to view the source code. There are also no plugin commands added, though there really should have been. All notetags and most plugin parameters are documented thoroughly.

Fun Fact: Internally, this plugin tries to make it look like its added features are part of the base engine, even if you won't see them in the editor. Traits and effects with custom values are added to database items. It can add stats to determine the limit of each skill currency ("max mp"), it even adds stats for regen and an Effect for skills and items!

## sMS_Jay_Dualtech_Ext
An extension plugin for Jay's Dualtechs (as the name states), this allows you to specify icons for Dualtechs, as well as take all members into account for crit rate and attack states applied. If you have Yanfly's Battle Engine installed, you can even dynamically change which members are considered via Action Sequences to provide for complex multi-character combos.

I took inspiration from Jeneeus Guruman's above ChangeGraphic plugin for the icon code, but added some more customizability. The icon code, crit rate code and Action Sequences were tested. I can also confirm the crit rate code works with Yanfly's Critical Core as long as the compatibility instructions are followed. There is some extra functionality when combined with my Multiple Elements and Moogle_X's Element Boost plugin that wasn't tested, more on that in the Multiple Elements section.

This plugin has a bit of a funny development history, but maybe I'll touch on that after I've finished listing the other plugins.

## sMS_MultipleElements
Honestly, I'm not sure I can recommend this plugin unless you want an element system to be the *core* of your battle system. It's just a bit too complicated for its own good, I wanted to make a system that made more "sense" but anyway.
This plugin allows you to assign multiple elements to a skill, and assign weights to each one. If you assign no weights, every element will be equally weighted, which effectively averages the element rates. This can be useful if you want to have a character, for example, infuse their weapon with multiple elements, but no matter how many they infuse, their sword skills will always be weighted half slashing damage. There are additional options for specifying how Normal Attack Elements are treated regarding automatic weighting, limiting the sources of Normal Attack Elements active at once and modifying Moogle_X's Element Boost plugin to use these calculations. Finally, you can use Action Sequences with Battle Engine Core to dynamically change elements.

I never ended up using Element Boost, so that functionality has gone untested. Naturally the extended functionality when paired with Element Boost plus sMS_Jay_Dualtech_Ext wasn't tested either. When all four (counting Jay's Dualtechs) plugins are used together, it adds an option for how to handle dualtechs where the members bring different Normal Attack Elements and Element Boosts. There is an advanced mode where only the Element Boosts applied to the same members who have the corresponding Normal Attack Elements are applied to the final multiplier. Distributing Normal Attack Elements into the full element list is not compatible with this mode. This mode does not affect elements which are explicitly assigned to the skill.

# Never Tested
These are plugins I never got around to testing. Features a lot of extension/"compatibility*" plugins. Younger me had an awful lot of hubris, and would like to believe most of these will work as is. Somewhat-older me at least has the sense to give you a warning. I'll order them by how simple they are, and how likely they are to work (a.k.a. by my faith and hubris).
<sub>*no compatibility promised *cough*</sub>
